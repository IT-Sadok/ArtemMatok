...
) : ControllerBase
{
    ...
    [Authorize(Roles = UserRoles.Host)]
    [HttpPut("{apartamentId}")]
    public async Task<IActionResult> UpdateCustomData(int apartamentId, [FromBody] List<ApartamentCustomDataDto> newCustomData)
    {
        var result = await _apartamentService.UpsertCustomData(apartamentId, newCustomData, UserHelpers.GetUserId(HttpContext));
